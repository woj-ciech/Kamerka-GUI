// easy d3v3-based word cloud plugin https://github.com/wvengen/d3v3-wordcloud
// requires https://github.com/jasondavies/d3v3-cloud
// based on https://github.com/shprink/d3v3js-wordcloud
(function() {
  function wordcloud() {
    var selector = '#wordcloud',
        element = d3v3.select(selector),
        transitionDuration = 200,
        scale = 'sqrt',
        fill = d3v3.scale.category20b(),
        layout = d3v3.layout.cloud(),
        fontSize = null,
        svg = null,
        vis = null,
    ...
    wordcloud.selector = function(x) {
      ...
      element = d3v3.select(x == null ? selector : x);
      return wordcloud;
    };
    ...
    wordcloud.fill = function(x) {
      ...
      fill = x == null ? d3v3.scale.category20b() : x;
      return wordcloud;
    };
      ...
      text.style("font-family", function(d) {
        ...
        .on("mouseover", function(d, i) {
          if (onwordclick !== undefined) {
            d3v3.select(this).transition().style('font-size', d.size + 3 + 'px');
          }
        })
        .on("mouseout", function(d, i) {
          if (onwordclick !== undefined) {
            d3v3.select(this).transition().style('font-size', d.size + 'px');
          }
        })
    ...
    function update() {
      ...
      fontSize = d3v3.scale[scale]().range([10, 100]);
      if (words.length) {
        fontSize.domain([+words[words.length - 1].size || 1, +words[0].size]);
      }
    }

    return d3v3.rebind(wordcloud, layout, 'on', 'words', 'size', 'font', 'fontStyle', 'fontWeight', 'spiral', 'padding');
  }

  if (typeof module === "object" && module.exports) module.exports = wordcloud;
  else d3v3.wordcloud = wordcloud;
})();
